...
class D3VController
	...
	def queryRest(queryStr)
		HTTPClient.new.get_content(
			'https://' + @sfPartner.url.host + '/services/data/v' + @defaultVersion + '/query?q=' + CGI::escape(queryStr), 
			'', 
	...
	def queryTooling(queryStr)
		begin
			result = HTTPClient.new.get_content(
				'https://' + @sfPartner.url.host + '/services/data/v' + @defaultVersion + '/tooling/query?q=' + CGI::escape(queryStr), 
				'', 
				{'Authorization' => 'OAuth ' + @sfPartner.session_id})
